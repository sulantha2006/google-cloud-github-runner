...
|------|-------------|------|---------|:--------:|
| <a name="input_apis"></a> [apis](#input\_apis) | List of Google Cloud APIs to be enable | `list(string)` | <pre>[<br/>  "artifactregistry.googleapis.com",<br/>  "cloudbuild.googleapis.com",<br/>  "cloudresourcemanager.googleapis.com",<br/>  "cloudscheduler.googleapis.com",<br/>  "cloudtasks.googleapis.com",<br/>  "compute.googleapis.com",<br/>  "iam.googleapis.com",<br/>  "logging.googleapis.com",<br/>  "monitoring.googleapis.com",<br/>  "orgpolicy.googleapis.com",<br/>  "run.googleapis.com",<br/>  "secretmanager.googleapis.com",<br/>  "storage.googleapis.com"<br/>]</pre> | no |
| <a name="input_github_runners_alert_heartbeat_missing_seconds"></a> [github\_runners\_alert\_heartbeat\_missing\_seconds](#input\_github\_runners\_alert\_heartbeat\_missing\_seconds) | Alert when no reconcile pass has completed for this many seconds | `number` | `900` | no |
| <a name="input_github_runners_alert_stuck_job_seconds"></a> [github\_runners\_alert\_stuck\_job\_seconds](#input\_github\_runners\_alert\_stuck\_job\_seconds) | Alert when a queued job has waited for a runner longer than this many seconds | `number` | `1800` | no |
| <a name="input_apis"></a> [apis](#input\_apis) | List of Google Cloud APIs to be enable | `list(string)` | <pre>[<br/>  "artifactregistry.googleapis.com",<br/>  "cloudbuild.googleapis.com",<br/>  "cloudresourcemanager.googleapis.com",<br/>  "cloudscheduler.googleapis.com",<br/>  "compute.googleapis.com",<br/>  "iam.googleapis.com",<br/>  "logging.googleapis.com",<br/>  "orgpolicy.googleapis.com",<br/>  "run.googleapis.com",<br/>  "secretmanager.googleapis.com",<br/>  "storage.googleapis.com"<br/>]</pre> | no |
| <a name="input_github_runners_auto_template_prefix"></a> [github\_runners\_auto\_template\_prefix](#input\_github\_runners\_auto\_template\_prefix) | Template name prefix for the gcp-auto failover ladder rungs (<prefix>-compute-16, <prefix>-general-2, ...) | `string` | `"gcp-ubuntu-24-04"` | no |
...
| <a name="input_github_runners_manager_max_instance_count"></a> [github\_runners\_manager\_max\_instance\_count](#input\_github\_runners\_manager\_max\_instance\_count) | GitHub Actions Runners manager app maximum instance count (Max. number of Cloud Run instances) | `number` | `10` | no |
...
| <a name="input_github_runners_manager_request_timeout"></a> [github\_runners\_manager\_request\_timeout](#input\_github\_runners\_manager\_request\_timeout) | Cloud Run request timeout in seconds for the GitHub Actions Runners manager (covers the longest provisioning task) | `number` | `1800` | no |
| <a name="input_github_runners_provision_max_attempts"></a> [github\_runners\_provision\_max\_attempts](#input\_github\_runners\_provision\_max\_attempts) | Cloud Tasks attempts per provisioning task before it is left to the reconciler (30 s to 300 s backoff) | `number` | `8` | no |
| <a name="input_github_runners_provision_max_concurrent_dispatches"></a> [github\_runners\_provision\_max\_concurrent\_dispatches](#input\_github\_runners\_provision\_max\_concurrent\_dispatches) | Cloud Tasks concurrent provisioning tasks in flight (each blocks on one Compute insert operation) | `number` | `20` | no |
| <a name="input_github_runners_provision_max_dispatches_per_second"></a> [github\_runners\_provision\_max\_dispatches\_per\_second](#input\_github\_runners\_provision\_max\_dispatches\_per\_second) | Cloud Tasks dispatch rate for runner provisioning (tasks started per second) | `number` | `5` | no |
| <a name="input_github_runners_manager_setup_password"></a> [github\_runners\_manager\_setup\_password](#input\_github\_runners\_manager\_setup\_password) | Password for the GitHub Actions Runners manager /setup pages (stored in Secret Manager) | `string` | n/a | yes |
| <a name="input_github_runners_manager_setup_username"></a> [github\_runners\_manager\_setup\_username](#input\_github\_runners\_manager\_setup\_username) | Username for the GitHub Actions Runners manager /setup pages (stored in Secret Manager) | `string` | n/a | yes |
| <a name="input_github_runners_reconcile_attempt_deadline"></a> [github\_runners\_reconcile\_attempt\_deadline](#input\_github\_runners\_reconcile\_attempt\_deadline) | Cloud Scheduler attempt deadline in seconds for one reconcile pass (max. 1800) | `number` | `600` | no |
| <a name="input_github_runners_reconcile_exclude_repositories"></a> [github\_runners\_reconcile\_exclude\_repositories](#input\_github\_runners\_reconcile\_exclude\_repositories) | Repositories (owner/repo) the reconciler neither provisions for nor cleans up; every other repository of the GitHub App installation is scanned | `list(string)` | `[]` | no |
| <a name="input_github_runners_reconcile_schedule"></a> [github\_runners\_reconcile\_schedule](#input\_github\_runners\_reconcile\_schedule) | Cloud Scheduler cron schedule (UTC) for the reconcile pass of the GitHub Actions Runners manager; must be of the form */N * * * * | `string` | `"*/5 * * * *"` | no |
| <a name="input_github_runners_reconcile_slow_retry_hours"></a> [github\_runners\_reconcile\_slow\_retry\_hours](#input\_github\_runners\_reconcile\_slow\_retry\_hours) | Hours after which the reconciler retries a still-queued job only every slow-retry interval instead of every pass | `number` | `6` | no |
| <a name="input_github_runners_reconcile_slow_retry_minutes"></a> [github\_runners\_reconcile\_slow\_retry\_minutes](#input\_github\_runners\_reconcile\_slow\_retry\_minutes) | Interval in minutes between provisioning attempts for jobs queued longer than the slow-retry hours | `number` | `60` | no |
| <a name="input_github_runners_reconcile_stuck_minutes"></a> [github\_runners\_reconcile\_stuck\_minutes](#input\_github\_runners\_reconcile\_stuck\_minutes) | Minutes a queued job may wait without a VM (or a VM may exist without a running job) before the reconciler acts | `number` | `10` | no |
